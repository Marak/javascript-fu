...
format.time.TimeSpan = function (days,hours,minutes,seconds,milliseconds){this.days=0;this.hours=0;this.minutes=0;this.seconds=0;this.milliseconds=0;if(arguments.length==5){this.days=days;this.hours=hours;this.minutes=minutes;this.seconds=seconds;this.milliseconds=milliseconds;}
...
format.time.compare = function (timeSpan){var t1=new Date(1970,1,1,this.hours(),this.minutes(),this.seconds()),t2;if(timeSpan===null){t2=new Date(1970,1,1,0,0,0);}
  else{t2=new Date(1970,1,1,timeSpan.hours(),timeSpan.minutes(),timeSpan.seconds());}
  return(t1>t2)?1:(t1<t2)?-1:0;};

format.time.add = function (timeSpan){return(timeSpan===null)?this:this.addSeconds(timeSpan.getTotalMilliseconds()/1000);};

format.time.subtract = function (timeSpan){return(timeSpan===null)?this:this.addSeconds(-timeSpan.getTotalMilliseconds()/1000);};

format.time.addDays = function (n){return new exports.TimeSpan(this.getTotalMilliseconds()+(n*24*60*60*1000));};

format.time.addHours = function (n){return new exports.TimeSpan(this.getTotalMilliseconds()+(n*60*60*1000));};

format.time.addMinutes = function (n){return new exports.TimeSpan(this.getTotalMilliseconds()+(n*60*1000));};

format.time.addSeconds = function (n){return new exports.TimeSpan(this.getTotalMilliseconds()+(n*1000));};

format.time.addMilliseconds = function (n){return new exports.TimeSpan(this.getTotalMilliseconds()+n);};

format.time.getTotalMilliseconds = function (){return(this.days()*(24*60*60*1000))+(this.hours()*(60*60*1000))+(this.minutes()*(60*1000))+(this.seconds()*(1000));};

format.time.get12HourHour = function (){return((h=this.hours()%12)?h:12);};

format.time.getDesignator = function (){return(this.hours()<12)?Date.CultureInfo.amDesignator:Date.CultureInfo.pmDesignator;};

format.time.toString = function (format){function _toString(){if(this.days()!==null&&this.days()>0){return this.days()+"."+this.hours()+":"+p(this.minutes())+":"+p(this.seconds());}
